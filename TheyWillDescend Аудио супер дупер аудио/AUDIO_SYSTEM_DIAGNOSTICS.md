...
**Статус:** ✅ РАБОТАЕТ — глобальный Distance-RTPC подключён (ветер), параметры идут по ID
...
### Проблема 3: Имя параметра не совпадает (в т.ч. невидимый пробел)
**Статус:** ✅ НАЙДЕНО И ИСПРАВЛЕНО (2026-09-07)  
**Симптом:** `setParameterByName("Distance")` возвращает OK, но readback даёт `ERR_EVENT_NOTFOUND`, значение всегда 0.

**Реальная причина:** параметр в FMOD-проекте назывался **`Distance ` — с хвостовым пробелом**. Точный матч имени в FMOD строгий: «Distance» ≠ «Distance » → параметр не найден. Обнаружено дампом всех глобальных параметров через `getParameterDescriptionList` (имя печатать как `(string)desc.name`, иначе видно `FMOD.StringWrapper`).

**Правильное решение (реализовано в GlobalAmbienceManager):**
1. При старте получить `getParameterDescriptionList` → сравнить имена с `Trim()` и `OrdinalIgnoreCase` → сохранить `PARAMETER_ID`.
2. Установка/чтение — только по ID: `setParameterByID(id, value, ignoreseekspeed: true)` / `getParameterByID`.
3. После set — readback и сверка (`sent X / readback Y`); при расхождении — автодамп параметров.
4. В FMOD Studio убрать хвостовой пробел из имени параметра (гигиена), но код больше от этого не зависит.

**Диагностический дамп (уже в коде, DumpGlobalParameters):**
```
FMOD has N global parameter(s):
  [0] 'Distance ' min=0 max=1 type=GAME_CONTROLLED current=0.000
```
Если имя в дампе отличается от ожидаемого (пробел, регистр, другое слово) — это и есть причина молчания RTPC.
...
### 5. Имя параметра не совпадает (включая невидимый пробел)
**Симптом:** set возвращает OK, readback `ERR_EVENT_NOTFOUND`, значение всегда 0  
**Решение:** Дампнуть параметры (`DumpGlobalParameters` в GlobalAmbienceManager), сравнить имя посимвольно (пробелы!). В коде работать по `PARAMETER_ID` с резолвом через `Trim()` — тогда опечатки в FMOD не ломают связку. В FMOD Studio имя поправить до чистого `Distance`.

### 5а. Seek Speed растягивает значение
**Симптом:** set проходит, но readback сразу после set даёт старое/нулевое значение  
**Решение:** `setParameterByID(id, value, ignoreseekspeed: true)` или Seek Speed = 0 в FMOD Studio (Edit Parameter).

### 5б. Параметр ставится в «scoped»-двойник
**Симптом:** глобальный параметр в дампе есть, но кривая не двигается  
**Решение:** глобальный параметр ставить через `RuntimeManager.StudioSystem.setParameterByID`, инстансный — через `instance.setParameterByID`. Не смешивать: у ивента может быть локальный параметр с тем же именем.
...
### Как проверить, что RTPC реально доходит (readback-паттерн)
1. В коде после `setParameterByID` сразу читать `getParameterByID` и логировать: `sent X / readback Y`.
2. `sent == readback` → значение дошло; если звук не меняется — проблема в кривой automation в FMOD (лейн/bypass/не тот параметр).
3. `readback = 0` + `ERR_EVENT_NOTFOUND` → параметр по имени не найден (пробел/регистр/не глобальный) → дампнуть `DumpGlobalParameters` и сверить имя посимвольно.
4. `readback` ползёт к X медленно → Seek Speed у параметра ненулевой → `ignoreseekspeed: true` или Seek Speed = 0.

### Направление Distance для ветра (текущее)
- `invertDistance = true` в GlobalAmbienceManager: **центр города = 0 (тише), максимальное отдаление = 1 (громче)**.
- Значение сглаживается экспоненциально (`Smooth Speed`, по умолч. 4) — плавно, без ступенек.
- Debug: колесо мыши двигает Distance шагами 0.1 (скролл вверх = отдаление), работает только при фокусе на Game View.